...
class Board(object):
    ...
    # Take a turn and update the game board
    ...
    # Return all possible moves on the game board
    ...
    def is_over(self) -> typing.Tuple[bool, int]:
        ...
        for i in range(self.size):
            for j in range(self.size):
                if self.cells[i][j] == 0:
                    return False, 0
        # Otherwise all cells are taken and no player has won (i.e draw)
        return True, 0

    # Returns a hash value for the current state of the game board
    def hash(self) -> int:
        result = 0
        for i in range(self.size):
            for j in range(self.size):
