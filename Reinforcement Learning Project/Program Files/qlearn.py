...
class QAgent(Player):
    def __init__(self,
        ...
        # Create a qtable for each possible board state
    ...
    def get_move(self, board: Board) -> int:
        ...
        # Take random move epsilon percentage of the time
    ...
    def final_result(self, result: int):
        ...
        if self.training == False:
            return
        # Assign final_value based on result
        ...
        # Reverse history to start from end of game
        self.history.reverse()
        firstTime = True
        # Loop over history of moves
        for h in self.history:
            qvals = self.qtable[h[0]]
            if firstTime:  
                # First time through the loop, set the reward of the final move to result reward
            ...
            # Argmax of the current states action values
            next_max = max(qvals)
